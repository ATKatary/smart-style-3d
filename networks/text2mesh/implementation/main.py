...
def run_branched(args):
    ...
    i = 0
    final_dir = os.path.join(dir, f"{objbase}_final_style_{i}")
    iters_dir = os.path.join(dir, f"{objbase}_iters_style_{i}")
    while os.path.isdir(final_dir) and os.path.isdir(iters_dir): 
        i += 1
        final_dir = os.path.join(dir, f"{objbase}_final_style_{i}")
        iters_dir = os.path.join(dir, f"{objbase}_iters_style_{i}")
    ...
    for i in tqdm(range(args.n_iter)):
        ...
        if i % 100 == 0:
            report_process(args, iters_dir, i, loss, loss_check, losses, rendered_images)
            export_final_results(args, final_dir, losses, mesh, mlp, network_input, vertices, i)
...
def report_process(args, dir, i, loss, loss_check, losses, rendered_images):
    ...
    torchvision.utils.save_image(rendered_images, os.path.join(dir, '{} iters.jpg'.format(i)))
...
def export_final_results(args, dir, losses, mesh, mlp, network_input, vertices, i):
    with torch.no_grad():
        ...
        objbase, extension = os.path.splitext(os.path.basename(args.obj_path))
        mesh.export(os.path.join(dir, f"{i} iterations.obj"), color=final_color)
