...
export function MeshRenderer(props) {
    let mesh;
    const onSceneReady = (scene, meshInfo) => {
        ...
        SceneLoader.ImportMesh("", `${meshInfo.dir}`, `${meshInfo.name}${meshInfo.ext}`, scene, (meshes) => {mesh = meshes[0]; mesh.position = new Vector3(0, 0, 0); mesh.material = mat;});
    ...
    return (
        <div style={{width: "95%", height: "100%", outline: "1px solid #646464"}} className="box-shadow-4" id={props.id}>
            <SceneComponent antialias meshInfo={props.meshInfo} onSceneReady={onSceneReady} onRender={onRender} id={useSlotProps.id} style={{width: "100%", height: "100%", outline: "none"}}/>
        </div>
    )
}
